...
def transcribe_image(image_id: str, ctx) -> list[str]:
    ...
    if not validate_image_mime_type(file_mime_type):
        return [f"Lo siento, el formato de la imagen no es válido. Los formatos válidos son: jpeg, png y tiff. El formato de la imagen que enviaste es: `{file_mime_type}`"]
