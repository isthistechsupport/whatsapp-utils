...
def get_audio_file(audio_id: str, phone_number_id: str, sender: str) -> BytesIO:
    ...
    file_url, file_hash, file_mime_type, file_size = response_json['url'], response_json['sha256'], response_json['mime_type'], response_json['file_size']
    if not validate_mime_type(file_mime_type):
        process_text(
            phone_number_id,
            sender,
            f"Lo siento, el formato del audio no es válido. Los formatos válidos son: flac, mp3, mp4, mpeg, mpga, m4a, ogg, wav y webm. El formato del audio que enviaste es: ```{file_mime_type}```"
        )
    ...
    if file_size > 25 * 1024 * 1024:
        process_text(
            phone_number_id,
            sender,
            f"Lo siento, el tamaño del audio es muy grande. El tamaño máximo permitido es de 25MB. El tamaño del audio que enviaste es: ```{file_size} bytes, {file_size / (1024 * 1024)} MB```"
        )
        raise requests.HTTPError()
    ...
    if hashed_content != file_hash:
        process_text(
            phone_number_id,
            sender,
            f"Lo siento, el audio que enviaste está corrupto. Por favor, intenta enviarlo de nuevo. ```{hashed_content} != {file_hash}```"
        )
        raise requests.HTTPError()
    return BytesIO(file_response.content)


def process_audio(audio_id: str, phone_number_id: str, sender: str, client: OpenAI):
    with get_audio_file(audio_id, phone_number_id, sender) as audio_file:
